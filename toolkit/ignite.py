...
def crawl_scan(args, target_url_string, blacklist=[], filter_regex="404 Not Found"):
    ...
    for url in url_list:
        ...
        home_dir = get_home_dir()
        subprocess.run([f"{home_dir}/go/bin/ffuf -w 'wordlists/crawl_wordlist_{args.domain}.txt' -u {url}FUZZ -recursion -recursion-depth 4 -r -p 0.1-3.0 -sa -t 50 -fr '{filter_regex}' -fc 403,401 -o temp/ffuf-results.tmp -of json"], shell=True)
        thisUrl = get_target_url_object(args, target_url_string)
        with open('temp/ffuf-results.tmp') as json_file:
            data = json.load(json_file)
        for result in data['results']:
...
def update_url(args, thisUrl):
    res = requests.post(f'http://{args.server}:{args.port}/api/url/auto/update', json=thisUrl, headers={'Content-type':'application/json'})
